...
public class BankController {
	...
	private static Logger logger = Logger.getLogger(com.cg.project.controller.BankController.class);
	...
	public String addUser(@RequestParam("usernamesignup") String userName,
			...
			@RequestParam("secretquestion") String secretQuestion, @RequestParam("secretanswer") String secretAnswer,
			Model model) {
		try {
			UserBean userFetched = service.fetchUserById(userName);
			if (userFetched == null) {
				service.registerUser(Constants.initialAccountId, userName, password, transactionPassword,
						Constants.lockStatus, secretQuestion, secretAnswer);
				model.addAttribute(Constants.message, Constants.SignUpDone);
				model.addAttribute(Constants.page, Constants.returnToHome);
				logger.info(Constants.SignUpDone);
				return RequestPage.redirectToPage;
			}
			model.addAttribute(Constants.message, Constants.alreadySignedUp);
			model.addAttribute(Constants.page, Constants.returnToHome);
			logger.error(Constants.alreadySignedUp);
	...
	public String validateAdmin(@RequestParam("admin-username") String adminId,
		...
		try {
			...
			if (count != 0) {
				...
				model.addAttribute(Constants.page, Constants.returnToHome);
				model.addAttribute(Constants.selectAdminStatement, Constants.checkforAdminStatement);
				logger.info(Constants.adminLogin);
				return RequestPage.redirectToPage;
			}
			model.addAttribute(Constants.message, Constants.adminInvalidCredentials);
			model.addAttribute(Constants.page, Constants.returnToHome);
			logger.info(Constants.adminInvalidCredentials);
			return RequestPage.redirectToErrorPage;
		} catch (BankingException be) {
			model.addAttribute(Constants.message, be.getMessage());
			model.addAttribute(Constants.page, Constants.returnToHome);
	...
	public String openAccount(@RequestParam("openFormUserName") String customerName,
		...
		try {
			...
			logger.info(Constants.accountOpen);
			return RequestPage.redirectToPage;
		} catch (BankingException be) {
			model.addAttribute(Constants.message, be.getMessage());
			model.addAttribute(Constants.page, Constants.returnToLogin);
	...
	public String changeRequest(@RequestParam("ft-newAddress") String newAddress,
		...
		try {
			...
			model.addAttribute(Constants.page, Constants.returnToLogin);
			logger.info(Constants.addressChanged);
			return RequestPage.redirectToPage;
		} catch (BankingException be) {
			model.addAttribute(Constants.message, be.getMessage());
			model.addAttribute(Constants.page, Constants.returnToLogin);
	...
	/**
	 * This method fetches transaction record for the payer and is called in
	 * fundtransfer method
	 * 
	 * @param accountId
	 * @param amount
	...
	private TransactionsBean getTransactionRecordForPayer(long accountId, double amount,
		...
		TransactionsBean transactionByActId = new TransactionsBean(transactionDescription,
				Date.valueOf(LocalDate.now()), Constants.transactionTypeForPayer, amount, accountId);
		return transactionByActId;
	}

	/**
	 * This method fetches transaction record for the payee and is called in
	 * fundtransfer method
	 * 
	 * @param payeId
	 * @param amount
	...
	private TransactionsBean getTransactionsRecordForPayee(long payeId, double amount, String transactionDescription) {
		TransactionsBean transactionByPayeeId = new TransactionsBean(transactionDescription,
				Date.valueOf(LocalDate.now()), Constants.transactionTypeForPayee, amount, payeId);
		return transactionByPayeeId;
	}

	/**
	 * This method fetches paytee table record for the payee and is called in
	 * fundtransfer method
	 * 
	 * @param accountId
	 * @param payeeId
	...
	/**
	 * This method fetches fund transfer record for the payee and is called in
	 * fundtransfer method
	 * 
	 * @param accountId
	 * @param payeeId
		...
		try {
			...
			if (payerId == accId) {
				...
				if (customerByActId != null) {
					...
					if (balance >= amount) {
						TransactionsBean transactionByActId = getTransactionRecordForPayer(accId, amount,
								transactionDescription);
						service.insertTransactionDetails(transactionByActId);
						TransactionsBean transactionByPayeeId = getTransactionsRecordForPayee(payeId, amount,
								transactionDescription);
	...
	public String viewDetailedStatement(@RequestParam("miniAccountNumber") long accountId,
		...
		try {
			...
			if (accountIdForUser == accountId) {
				...
				logger.info(Constants.detailedStatement);
				return RequestPage.redirectToListPage;
			}
			map.addAttribute(Constants.message, Constants.invalidAccountId);
			map.addAttribute(Constants.page, Constants.returnToLogin);
	...
	public String adminViewTransactions(@RequestParam("accountId") long accountId, Model map) {
		try {
			...
			map.addAttribute(Constants.selectAdminStatement, Constants.checkforAdminStatement);
			map.addAttribute(Constants.noOfTransactions, adminStatement.size());
			map.addAttribute(Constants.page, Constants.returnToHome);
			logger.info(Constants.transactionsForAdmin);
			return RequestPage.redirectToListPage;
		} catch (BankingException be) {
			map.addAttribute(Constants.message, be.getMessage());
			map.addAttribute(Constants.page, Constants.returnToHome);
	...
	public String changePassword(@RequestParam("userName") String userName,
		...
		try {
			...
			if (loginPass.equals(oldpassword)) {
				...
				model.addAttribute(Constants.page, Constants.returnToHome);
				logger.info(Constants.LoginPasswordChanged);
				return RequestPage.redirectToPage;
			}
			model.addAttribute(Constants.message, Constants.incorrectOldPassword);
			model.addAttribute(Constants.page, Constants.returnToHome);
			logger.error(Constants.incorrectOldPassword);
			return RequestPage.redirectToErrorPage;
		} catch (BankingException be) {
			model.addAttribute(Constants.message, be.getMessage());
			model.addAttribute(Constants.page, Constants.returnToHome);
